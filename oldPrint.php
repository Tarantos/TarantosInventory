...
<body>
<?php
$inventoryToPrint = $_POST['inventoryToPrint'];
	$subInv = substr($inventoryToPrint, 9, 8);
	$regName = ereg_replace("_", "/", $subInv);

	print "Inventory from ".$regName;
	...
	$foodCost = $BreadTotalCost + $CheeseTotalCost + $DessertTotalCost + $DressingTotalCost + $DryTotalCost + $FryTotalCost +$LettuceTotalCost + $MeatTotalCost + $PastasTotalCost + $SauceTotalCost + $SoupTotalCost + $VegetableTotalCost;
       	$nonFoodCost = $BoxesTotalCost + $CleaningTotalCost + $ContainersTotalCost + $PaperTotalCost;
	print "<h4>Summary of Inventory for $regName</h4>";
	print "<table border='1px'>";
	print '<tr>';
	...
	print '</tr>';	
	print '<tr>';
	print '<td>Food Cost</td>';
	print "<td>$foodCost</td>";
	print '</tr>';
	print '<tr>';
	print '<td>Non Food Cost</td>';
	print "<td>$nonFoodCost</td>";
	print '</tr>';
	print "<tr><td>Total Cost</td><td>";
	print $BudTotalCost + $CokeTotalCost + $foodCost + $nonFoodCost;
	print "</td></tr>";
